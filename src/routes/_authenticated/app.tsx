...
function Workspace() {
  ...
  const greeting = mode === "plan"
    ? "Let's shape the plan"
    : firstName
      ? `What should we build, ${firstName.charAt(0).toUpperCase() + firstName.slice(1)}?`
      : "What should we build?";

  const words = input.trim() ? input.trim().split(/\s+/).length : 0;
  const overLimit = words > MAX_WORDS;

  /** Animated switch between Plan and Build, optionally pre-filling the input. */
  function switchMode(next: "plan" | "build", prefill?: string) {
  ...
  const composer = (
    <form onSubmit={send} className="animate-fade-in-up">
      <div
        className={`rounded-2xl glass-strong border transition-colors ${overLimit ? "border-destructive/60" : "border-glass-border focus-within:border-primary/50"} shadow-lift`}
      >
        {!!attachments.length && (
          <div className="flex flex-wrap gap-2 px-3 pt-3">
            {attachments.map((src, i) => (
              <div key={i} className="relative">
                <img src={src} alt={`Attachment ${i + 1}`} className="h-14 w-14 rounded-lg object-cover border border-glass-border" />
                <button
                  type="button"
        ...
        <textarea
          ...
          rows={2}
          placeholder={mode === "plan" ? "Describe the idea — Kenzo plans it first…" : "Describe what to build or change…"}
          className="w-full resize-none bg-transparent px-4 pt-3.5 pb-2 text-sm outline-none placeholder:text-muted-foreground/70 max-h-52"
        />
        ...
        <div className="flex items-center gap-1 px-2.5 pb-2.5">
          <div className="flex rounded-lg bg-input p-0.5">
            {([["build", Hammer, "Build"], ["plan", ClipboardList, "Plan"]] as const).map(([k, Icon, label]) => (
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium transition-all duration-300 active:scale-95 ${mode === k ? "gradient-brand text-primary-foreground shadow-lift" : "text-muted-foreground hover:text-foreground"}`}
              >
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} title="Attach images" aria-label="Attach images" className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-surface transition active:scale-95">
          ...
          <button
            ...
            className={`p-2 rounded-lg transition active:scale-95 ${recording ? "bg-destructive/15 text-destructive" : "text-muted-foreground hover:text-foreground hover:bg-surface"}`}
          >
            {transcribing ? <Loader2 className="h-4 w-4 animate-spin" /> : recording ? <Square className="h-4 w-4" /> : <Mic className="h-4 w-4" />}
          </button>

          <span className={`ml-auto mr-1 text-[11px] tabular-nums ${overLimit ? "text-destructive" : "text-muted-foreground/70"}`}>
          ...
          <button
            ...
            aria-label={mode === "plan" ? "Send to planner" : "Build"}
            className="inline-flex h-9 w-9 items-center justify-center rounded-xl gradient-brand text-primary-foreground shadow-lift transition active:scale-95 disabled:opacity-40"
          >
            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
          </button>
        </div>
      </div>
      {overLimit && <p className="mt-1.5 px-1 text-[11px] text-destructive">That's over the {MAX_WORDS.toLocaleString()}-word limit — trim it a little.</p>}
    </form>
  );
